...
def inference_consumer():
    ...
    try:
        consumer = KafkaConsumer(
            ...
            group_id=f'inference-consumer-group-{producer_id}',
        ...
        for message in consumer:
            ...
            if result_producer_id == producer_id:
                result_id = data['ID']  
...
# Once all messages are processed, plot the latency histogram
plt.hist(latency_list, bins=50, edgecolor='black')
plt.title("Latency Distribution")
plt.xlabel("Latency (seconds)")
plt.ylabel("Frequency")
plt.savefig("latency_histogram.png")
print("Latency histogram saved as 'latency_histogram.png'.")